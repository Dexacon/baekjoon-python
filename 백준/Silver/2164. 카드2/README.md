...
### 출력 
 ...
 ### 🧠 배운 점 & 설계 과정
1. **문제 해석**: 
   - 1부터 N까지의 번호가 붙은 N장의 카드를 준비한다.
   - 제일 위에 1번카드가 있고 밑으로는 순서대로 카드가 놓여있다.
   - 카드가 한장 남을때까지 맨 위의 카드는 버리고 그 다음 맨 위의 카드는 카드 맨 아래로 옮긴다.
   - range로 리스트를 만들면 1번카드가 리스트 맨 앞으로 오기 때문에 큐의 popleft를 이용해서 푸는것으로 보인다.
2. **설계**:
   - list(range(1,N+1)로 카드 덱을 만든다.
   - 맨 위의 값인 1부터 popleft로 빼고 그 다음 값은 popleft로 뺀 후 append로 뒤에 다시 추가한다.
   - while len > 1 으로 한장 남을때까지 반복한다.
3. **실수 & 해결**:
   - 실수 없었다.

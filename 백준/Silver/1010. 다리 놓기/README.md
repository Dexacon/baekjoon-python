...
### 🧠 배운 점 & 설계 과정
1. **문제 해석**: 
   - 도시를 동쪽과 서쪽으로 나누는 강 사이에 다리를 놓는다.
   - 동쪽과 서쪽의 사이트를 연결해 다리를 건설하려고 하지만 동쪽의 사이트 개수가 서쪽의 사이트 개수 보다 많거나 같다.
   - 서쪽과 동쪽의 사이트를 연결하는 경우의수를 구하여라.
   - 조합의 개념을 이용해야 할것 같다.(다리를 겹치게 지을 수 없기 때문이다.)
2. **설계**:
   - 조합의 개념을 사용하여 m개중에 n개를 순서없이 뽑는다.
   - MCN은 M!/N!(M-N)!이기 때문에 식을 계산하여 답을 구한다.
3. **실수 & 해결**:
   - 조합에 대해 학습할 수 있었다.

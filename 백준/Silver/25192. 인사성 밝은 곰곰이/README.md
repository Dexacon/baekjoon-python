...
### 출력 
 ...
 ### 🧠 배운 점 & 설계 과정
1. **문제 해석**: 
   - ENTER가 입력되면 새로운 사람이 입장했음을 의미한다.
   - 그 이후 입력되는 문자열은 채팅을 입력한 유저의 이름이다.
   - 새로운 사람이 입장한 이후 처음 채팅을 입력하는 사람은 반드시 곰곰티콘으로 인사를 한다.
   - 곰곰티콘이 사용된 횟수를 구하여라.
   - 시간 제한을 고려하여 집합을 이용하여 탐색해야 할것같다.
2. **설계**:
   - ENTER가 입력되면 집합이 생성되도록한다.(후에 ENTER가 한번 더오면 초기화 되도록)
   - cnt를 이용해 집합안에 입력값이 없으면 cnt += 1을 한다.
   - 이렇게 설계하면 ENTER가 한번더 입력된다면 집합이 초기화되어 곰곰티콘을 입력했던 사람도 다시 곰곰티콘을 입력해야하는 것을 셀 수 있다.
3. **실수 & 해결**:
   - 실수 없었다.

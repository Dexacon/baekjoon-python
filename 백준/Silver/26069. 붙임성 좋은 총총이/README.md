...
1. **문제 해석**: 
   - 총총이는 무지개 댄스를 추고있다.
   - 무지개 댄스를 추고있는 사람을 보게되면 그 사람도 무지개 댄스를 추게된다.
   - 마지막 입력이후 무지개 댄스를 추고있는 사람의 수를 구하여라.
   - 곰곰이 문제와 마찬가지로 집합을 이용할것 같다.
2. **설계**:
   - 무지개 댄스를 추고있는 사람과 같이 입력되면 무지개 댄스를 추게 되기 때문에 시간을 단축하면서 in을 쓰기 위해 집합을 이용한다.
   - 입력 전에 무지개 댄스를 추고 있는 사람 목록에 총총이를 추가하고 시작한다.
   - 입력된 사람 중 첫 번째 사람이 추고 있는 사람 목록에 있고 두 번째 사람이 없다면 두 번째 사람을 추고 있는 사람 목록에 추가한다.(반대도 마찬가지로 진행한다.)
   - 둘다 안 추고 있거나 둘 다 추고 있는 경우는 변경 점이 없어서 고려할 필요 없다.
3. **실수 & 해결**:
   - 실수 없었다.

...
1. **문제 해석**: 
   - 병합 정렬을 재귀함수로 표현한다.
   - 병합정렬은 더 이상 쪼갤 수 없을때까지 반으로 쪼개서 정렬하고 합치는 구조이다.
2. **설계**:
   - 함수는 숫자로 된 리스트를 인수로 받는다.
   - if로 리스트의 길이가 1이 되었을때 리스트를 리턴하도록 한다.
   - 리스트를 반으로 쪼개고 그 쪼갠 리스트를 재귀함수에 다시 넣는다.
   - 마지막에는 merge함수를 리턴한다.
   - merge함수는 쪼개진 리스트들을 인수로 받아 두 리스트를 정렬하면서 합치는 기능을 한다.
   - merge_sort함수는 리스트의 길이가 1이 될때까지 계속해서 쪼개고 그 리스트를 정렬하면서 합치는걸 반복한다.
3. **실수 & 해결**:
   - 재귀함수의 답을 보고 과정을 이해하는것은 그나마 괜찮아지고 있으나 아직 재귀함수를 설계하는것은 매우 부족하다.
   - 여러가지 재귀함수를 풀어보고 답을 보고 이해하려고 노력해야겠다.

...
### 출력 
 ...
 <p>각 테스트케이스마다, isPalindrome 함수의 반환값과 recursion 함수의 호출 횟수를 한 줄에 공백으로 구분하여 출력한다.</p>
  ...
  ### 🧠 배운 점 & 설계 과정
1. **문제 해석**: 
   - 입력한 문자열이 팰린드롬인지 여부를 판단하여 팰린드롬이면 1, 팰린드롬이 아니면 0을 리턴하도록한다.
   - 재귀 함수를 이용하여 팰린드롬인지 여부를 판단하고 반환값과 재귀 함수의 호출 횟수를 출력하라.
2. **설계**:
   - 함수에서 문자열과 0 그리고 마지막 인덱스를 입력받도록 한다.
   - 호출 횟수를 세기 위해 cnt를 외부에서 선언하고 global로 전역 변수로 만든다.
   - l이 r보다 크거나 같아지면 문제가 없었다는 의미 이므로 1을 리턴한다.
   - 양쪽이 다를 경우엔 팰린드롬이 아니므로 0을 리턴한다.
   - 양쪽이 같을 경우엔 recursion(x,l+1,r-1)을 리턴하여 더욱 좁혀가면서 검사한다.(l이 r보다 크거나 같아지면 자동으로 1을 리턴 후 재귀 종료)
   - 각 케이스에 cnt += 1을 넣어서 recursion 함수의 호출 횟수를 저장한다.
3. **실수 & 해결**:
   - 아직 재귀로 문제 푸는것이 익숙하지 않아서 더욱 열심히 학습해야 할것같다.
